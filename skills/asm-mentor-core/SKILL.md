...
## 실행 방법
플러그인 설치 시 cwd와 무관하게 항상 절대경로로 호출된다:
```
node "${CLAUDE_PLUGIN_ROOT}/skills/asm-mentor-core/scripts/asm.mjs" <command> --region seoul|busan [옵션]
```
- `${CLAUDE_PLUGIN_ROOT}`는 Claude Code가 플러그인 설치 위치로 자동 치환한다(스킬 문서 텍스트 어디서든 동작).
  로컬 개발 클론(비플러그인)에서는 이 부분을 `node skills/asm-mentor-core/scripts/asm.mjs`로 대체해 프로젝트
  루트에서 실행해도 된다. 스크립트는 `.env`를 스스로 찾아 세션/산출물 경로를 정한다.
...
### 최초 설정 (플러그인 설치 후 1회)
플러그인 자체에는 자격증명이 들어있지 않다 — `.env` 정보는 항상 각자 로컬(홈 디렉터리)에만 저장한다.
처음 한 번만 실행:
```
...
```
이후 `~/.config/asm-mentor/.env` 를 열어 `ASM_HOMEPAGE_ID`/`ASM_HOMEPAGE_PW` 등 실제 계정 정보를 입력한다.
이 파일은 플러그인 설치 경로 밖에 있으므로 플러그인 업데이트·재설치와 무관하게 유지된다(`layeredEnv()`가
가장 낮은 우선순위로 항상 여기서 읽는다 — `scripts/lib/env.mjs` 참조).
