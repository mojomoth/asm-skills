...
- **여러 명은 반드시 한 번에**: `--search` 는 쉼표로 여러 이름을 받는다 — `--search "안용수,문재윤,오혜린"`.
  명단 전체를 1회만 받아 메모리에서 거르므로, **한 팀의 팀원/멘토를 사람마다 따로 호출하지 말 것**(연속 호출은
  Notion 일시 차단(throttle)을 유발해 `NAV_ERROR`로 실패). 한 팀 조회는 `team` 1회 + `roster --kind mentees`(팀원 전부) 1회 +
  `roster --kind mentors`(멘토 전부) 1회, **총 3회**면 충분하다.
- `NAV_ERROR`(queryCollection 가로채기 실패)가 나면 페이지 구조 변경이 아니라 **대개 throttle** → 잠시 후 재시도하거나
  위처럼 쉼표 검색으로 호출 수를 줄인다.
